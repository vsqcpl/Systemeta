...
export interface AIPageProps {
  embeddedView?: "main" | "delay-dashboard" | "billing-dashboard" | "assignment-dashboard" | "clash-dashboard" | "wbs-center" | null;
  embeddedModal?: string | null;
  embeddedProjectId?: string | null;
  embeddedTaskId?: string | null;
...
export default function AIPageComponent({
  ...
  const [view, setView] = useState<"main" | "delay-dashboard" | "billing-dashboard" | "assignment-dashboard" | "clash-dashboard" | "wbs-center">(embeddedView || "main");
            ...
            <button 
              ...
              onClick={() => { 
                if (isInline) {
                  ...
                  setView("main"); setDelayScanData(null); setSelectedProjectId(""); setSelectedTaskForAi(null); setAiReport(null); 
                }
              }}
            >
              {isInline ? "Close AI" : "← Back to AI Center"}
            </button>
            <div>
            ...
            <button 
              ...
              onClick={() => { 
                if (isInline) {
                  ...
                  setView("main"); setBillingScanData(null); setBillingProjectId(""); setSelectedMilestoneForAi(null); setBillingAiReport(null); 
                }
              }}
            >
              {isInline ? "Close AI" : "← Back to AI Center"}
            ...
            <button 
              ...
              onClick={() => { 
                if (isInline) {
                  ...
                  setView("main"); setClashScanData(null); 
                }
              }}
            >
              {isInline ? "Close AI" : "← Back to AI Center"}
...
Output ONLY the JSON array. Do not include markdown formats.`;
            ...
            <button
              ...
              onClick={() => { 
                if (isInline) {
                  ...
                  setView("main"); setWbsDraft(null); setWbsAnalysis(null); 
                }
              }}
              style={{ display: "flex", alignItems: "center", gap: "6px" }}
            >
              {isInline ? "Close AI" : "← Back"}
            </button>
            <div>
            ...
            <button 
              ...
              onClick={() => { 
                if (isInline) {
                  ...
                  setView("main"); setAssignmentResult(null); setAssignmentProjectId(""); setAssignmentTaskId(""); 
                }
              }}
            >
              {isInline ? "Close AI" : "← Back to AI Center"}
  ...
  const handleExportSummaryPdf = async () => {
    try {
      ...
      doc.text("Systemeta Weekly AI Summary", 20, 25);
      ...
      doc.text("Systemeta Enterprise Platform · Confidential", 20, 276);
      doc.text("Page 1 of 1", 170, 276);
      const dateStr = new Date().toISOString().split("T")[0];
      doc.save(`Systemeta_AI_Summary_${dateStr}.pdf`);
  ...
  const [activeModal, setActiveModal] = useState<string | null>(embeddedModal || null);
  const [isInline, setIsInline] = useState(!!embeddedView || !!embeddedModal);
  const prevModalRef = React.useRef<string | null>(null);

  React.useEffect(() => {
    if (typeof window !== "undefined") {
      setIsInline(window.location.search.includes("inline=true") || !!embeddedView || !!embeddedModal);
    }
  }, [embeddedView, embeddedModal]);
  ...
  React.useEffect(() => {
    if (embeddedModal !== undefined) {
      setActiveModal(embeddedModal);
    }
  }, [embeddedModal]);

  React.useEffect(() => {
    if (embeddedProjectId) {
      setEstProjectId(embeddedProjectId);
      setPredProjectId(embeddedProjectId);
      setAssignmentProjectId(embeddedProjectId);
      setWbsProjectId(embeddedProjectId);
      setBillingProjectId(embeddedProjectId);
      setSelectedProjectId(embeddedProjectId);
      setClashProjectId(embeddedProjectId);
    }
  }, [embeddedProjectId]);

  React.useEffect(() => {
    if (embeddedTaskId) {
      setEstTaskId(embeddedTaskId);
      setPredTaskId(embeddedTaskId);
      setAssignmentTaskId(embeddedTaskId);
      const taskObj = getFlatTasks(data?.tasks).find((t: any) => t.id === embeddedTaskId);
      if (taskObj) {
        setEstTask(taskObj.title);
        setPredTaskName(taskObj.title);
        if (taskObj.project) {
          setEstProjectId(taskObj.project);
          setPredProjectId(taskObj.project);
          setAssignmentProjectId(taskObj.project);
          setWbsProjectId(taskObj.project);
          setBillingProjectId(taskObj.project);
          setSelectedProjectId(taskObj.project);
          setClashProjectId(taskObj.project);
        }
      }
    }
  }, [embeddedTaskId, data?.tasks]);

  React.useEffect(() => {
    if (isInline && activeModal === null && prevModalRef.current !== null) {
      if (onCloseEmbedded) {
        onCloseEmbedded();
      } else if (typeof window !== "undefined" && window.parent) {
        window.parent.postMessage({ type: 'close-ai-modal' }, '*');
      }
    }
    prevModalRef.current = activeModal;
  }, [activeModal, isInline, onCloseEmbedded]);

  React.useEffect(() => {
    if (isInline && view === "main" && !activeModal) {
      if (onCloseEmbedded) {
        onCloseEmbedded();
      } else if (typeof window !== "undefined" && window.parent) {
        window.parent.postMessage({ type: 'close-ai-modal' }, '*');
  ...
  }, [view, isInline, onCloseEmbedded, activeModal]);
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const qProjId = params.get("projectId");
      const qTaskId = params.get("taskId");
      const qWbsMode = params.get("wbsMode");

      if (qView) {
        setView(qView as any);
      }
      if (qModal) {
        setActiveModal(qModal);
      }
      if (qProjId) {
        setEstProjectId(qProjId);
        setPredProjectId(qProjId);
        setAssignmentProjectId(qProjId);
        setWbsProjectId(qProjId);
        setBillingProjectId(qProjId);
        setSelectedProjectId(qProjId);
        setClashProjectId(qProjId);
      }
      if (qTaskId) {
        setEstTaskId(qTaskId);
        setPredTaskId(qTaskId);
        setAssignmentTaskId(qTaskId);
        const taskObj = getFlatTasks(data?.tasks).find((t: any) => t.id === qTaskId);
        if (taskObj) {
          setEstTask(taskObj.title);
          setPredTaskName(taskObj.title);
        }
  ...
  React.useEffect(() => {
    if (isInline && selectedProjectId) {
      handleScanProjectDelays(selectedProjectId);
    }
  }, [selectedProjectId, isInline]);

  React.useEffect(() => {
    if (isInline && billingProjectId) {
      handleScanBillingMilestones(billingProjectId);
    }
  }, [billingProjectId, isInline]);

  React.useEffect(() => {
    if (isInline && assignmentProjectId && assignmentTaskId) {
      handleFetchAssignmentRecommendations(assignmentProjectId, assignmentTaskId);
    }
  }, [assignmentProjectId, assignmentTaskId, isInline]);

  React.useEffect(() => {
    if (isInline && clashProjectId && clashProjectId !== "all") {
      handleRunScheduleClashScan(clashProjectId);
    }
  }, [clashProjectId, isInline]);

  React.useEffect(() => {
    if (isInline && wbsProjectId) {
      handleLoadExistingWbs(wbsProjectId);
    }
  }, [wbsProjectId, isInline]);
    ...
    try {
      const res = await fetch("/api/ai/estimate-time", {
        ...
        body: JSON.stringify({
          ...
          priority: selectedEstTask?.priority || "medium",
          teamSize: "1",
  ...
  // Auto-run estimate if embeddedTaskId is provided and resolved
  React.useEffect(() => {
    if (activeModal === "estimate-tasks" && estTaskId && estProjectId && !estResult && !estLoading) {
      handleRunEstimate();
    }
  }, [activeModal, estTaskId, estProjectId]);
  ...
  function getFlatTasksLocal(tasksState: any): any[] {
    if (!tasksState) return [];
    if (Array.isArray(tasksState)) return tasksState;
    const flat: any[] = [];
    if (Array.isArray(tasksState.todo)) flat.push(...tasksState.todo);
  ...
  return (
    <div style={{ animation: "fadeIn 0.5s ease-out" }}>
      ...
      {activeModal === "estimate-tasks" && (
        <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
          <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(680px, 95%)", maxHeight: "90vh", overflowY: "auto", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "billing-insights" && (
        <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
          <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(520px, 95%)", maxHeight: "90vh", overflowY: "auto", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "billing-report" && (() => {
        ...
        return (
          <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
            <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(560px, 95%)", maxHeight: "85vh", overflowY: "auto", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "auto-assign" && (
        <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
          <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(500px, 95%)", maxHeight: "90vh", overflowY: "auto", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "view-assignments" && (() => {
        ...
        return (
          <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
            <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(540px, 95%)", maxHeight: "85vh", overflowY: "auto", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "scan-delays" && (
        <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
          <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(500px, 95%)", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
      ...
      {activeModal === "analyze-roots" && (() => {
        ...
        return (
          <div className={isInline ? "" : "modal-overlay"} style={isInline ? { width: "100%" } : { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={isInline ? undefined : () => setActiveModal(null)}>
            <div className={isInline ? "" : "modal-content"} style={isInline ? { background: "var(--bg-surface)", padding: "12px", width: "100%" } : { background: "var(--bg-surface)", borderRadius: "12px", padding: "24px", width: "min(460px, 90%)", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border-default)", animation: "slideDown 0.2s cubic-bezier(0.4,0,0.2,1)" }} onClick={(e) => e.stopPropagation()}>
